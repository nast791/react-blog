...
const AuthChoice = props => {
  ...
  const loginHandler = data => {
    props.auth(data.email, data.password, true);
  };

  const registerHandler = data => {
    props.auth(data.setEmail, data.setPassword, false, data.setNickname, data.setUsername);
  };
  ...
  return (
    <React.Fragment>
      ...
      <AuthChoiceTabs>
        <AuthChoiceTab onClick={clickEntryHandler} data-active={props.tabForm === 'entry'}>Войти</AuthChoiceTab>
        <AuthChoiceTab onClick={clickRegHandler} data-active={props.tabForm === 'reg'}>Зарегистрироваться</AuthChoiceTab>
      </AuthChoiceTabs>
      {props.tabForm === 'entry' ?
        <EntryForm onSubmit={loginHandler} errorText={props.error ? props.error.message : null} errorNum={props.error ? props.error.number : null}/> :
        <RegistrationForm onSubmit={registerHandler} errorText={props.error ? props.error.message : null} errorNum={props.error ? props.error.number : null}/>
      }
    </React.Fragment>
  );
};
...
function mapStateToProps(state) {
  const {error, tabForm} = state.auth;
